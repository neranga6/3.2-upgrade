...
<div class="group">
	...
	<div class="richTextPanel">
		...
		<div class="fieldcontain ${hasErrors(bean: moduleInstance, field: 'section', 'error')} required form-field">
			...
			<g:select id="section" name="section.id" from="${com.nationwide.edm.authoring.Section.list(sort: 'sectionName')}"  noSelection="['':'-- select section --']" optionKey="id" required="" value="${moduleInstance?.section?.id}" class="many-to-one"/>
		</div>
		...
		<div class="fieldcontain ${hasErrors(bean: moduleInstance, field: 'businessUnit', 'error')} form-field">
			...
			<g:select id="businessUnit" name="businessUnit.id" from="${com.nationwide.edm.authoring.BusinessUnit.list(sort: 'name')}"  noSelection="['':'-- select Business Unit --']" optionKey="id" required="" value="${moduleInstance?.businessUnit?.id}" class="many-to-one"/>
		</div>
